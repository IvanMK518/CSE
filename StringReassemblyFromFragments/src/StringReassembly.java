...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //chop off overlap from the first string then
        //Concatenate the second string with overlap.
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        for (int i = 0; i < len; i++) {
            String remove = strSet.removeAny();
            temp.add(remove);
            //this basically does nothing
            if (remove.indexOf(str) >= 0) {
                flag = true;
                //if the string added has existing substrings in the method
        ...
        //adds the strings read from file back new set
        //as long as there are no overlapping substrings
        if (!flag) {
            temp.add(str);
        }

        //transfers altered new set into strSet
        //thus updating strSet
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            //if set size is zero, then go ahead and add input
            if (testSet.size() == 0) {
                testSet.add(input.nextLine());
            } else {
                //else use method below to prevent duplicates
                //and proper substrings existing within same set
                addToSetAvoidingSubstrings(testSet, input.nextLine());
            }
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (text.charAt(i) != '~') {
                ...
                //if the code finds character ~ then it replaces it with a space
                out.println();
            }
        }
